<?php
    ...
    curl_setopt_array($curl, [
        CURLOPT_URL => $url_simrs . '/API/SIMRS/kunjungan.php',
    ...
    foreach ($list_kunjungan as $row) {
        ...
        // Cek Apakh bersangkutan sudah ada atau belum
        $id_respondent = GetDetailData($Conn, 'respondent', 'id_kunjungan', $id_kunjungan, 'id_respondent');
        if(empty($id_respondent)){
            $tombol_opsi = '
                <button type="button" class="btn btn-sm btn-primary modal_tambah_responden" data-id="'.$id_kunjungan.'">
                    <i class="bi bi-plus"></i> Tambah
                </button>
            ';
        }else{
            $tombol_opsi = '
                <button type="button" class="btn btn-sm btn-secondary modal_detail_responden" data-id="'.$id_respondent.'">
